-- Fix PGRST203 : fonction ambiguë user_sessions
-- Supprime toutes les versions de insert_user_session_secure et upsert_user_session_secure
-- Recrée une seule version avec p_row JSONB (paramètres envoyés par sync-manager.js)

-- Colonnes player_id, player_server, player_pseudo si absentes
...
COMMENT ON FUNCTION public.insert_user_session_secure(JSONB) IS 'Insertion session avec limite. Paramètres dans p_row : local_id, honor, xp, rank_points, player_id, player_server, player_pseudo.';
COMMENT ON FUNCTION public.upsert_user_session_secure(JSONB) IS 'Upsert session avec limite. Paramètres dans p_row : local_id, honor, xp, rank_points, player_id, player_server, player_pseudo.';
