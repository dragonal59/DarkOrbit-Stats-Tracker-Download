-- ==========================================
-- Comparaison historique des snapshots de classement
-- + Nettoyage automatique après 30 jours
-- ==========================================

-- -----------------------------------------------
-- 1. get_ranking_comparison
--    Retourne le classement le plus récent + le classement de référence
--    (snapshot le plus récent avant now() - p_hours heures),
--    avec les deltas par joueur.
-- -----------------------------------------------
...
BEGIN
  ...
  -- Cutoff = snapshot le plus récent AVANT (latest_at - p_hours heures)
...
-- -----------------------------------------------
-- 2. cleanup_old_ranking_snapshots
--    Supprime les snapshots de plus de 30 jours.
--    Appelée depuis le scraper après chaque run complet.
-- -----------------------------------------------
CREATE OR REPLACE FUNCTION public.cleanup_old_ranking_snapshots()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  v_deleted INT;
BEGIN
  DELETE FROM public.shared_rankings_snapshots
...
GRANT EXECUTE ON FUNCTION public.cleanup_old_ranking_snapshots() TO authenticated;
GRANT EXECUTE ON FUNCTION public.cleanup_old_ranking_snapshots() TO anon;
