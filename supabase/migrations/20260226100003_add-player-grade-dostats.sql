-- Grade DOStats dans player_profiles (extrait du profil joueur)

ALTER TABLE public.player_profiles ADD COLUMN IF NOT EXISTS grade TEXT DEFAULT NULL;
...
BEGIN
  INSERT INTO public.player_profiles (
    ...
    npc_kills, ship_kills, galaxy_gates, galaxy_gates_json, grade, dostats_updated_at
  )
  VALUES (
  ...
  ON CONFLICT (user_id, server) DO UPDATE SET
    ...
    dostats_updated_at = CASE
      WHEN EXCLUDED.npc_kills IS NOT NULL OR EXCLUDED.ship_kills IS NOT NULL OR EXCLUDED.galaxy_gates IS NOT NULL OR (EXCLUDED.galaxy_gates_json IS NOT NULL AND jsonb_typeof(EXCLUDED.galaxy_gates_json) = 'object') OR EXCLUDED.grade IS NOT NULL THEN NOW()
...
-- Merge grade from player_profiles dans get_ranking (si absent dans snapshot)
