-- ==========================================
-- SCHÉMA MESSAGERIE ADMIN → UTILISATEURS
...
-- Utilisateurs : lire leurs propres messages, mettre à jour is_read et deleted_by_user
CREATE POLICY "Users read own messages" ON admin_messages
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users update own messages" ON admin_messages
  FOR UPDATE USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- L'insertion se fait via RPC admin_send_message (vérifie le badge)

CREATE POLICY "Admins read sent messages" ON admin_messages
  FOR SELECT USING (
    auth.uid() = admin_id
    OR auth.uid() = user_id
  );

-- RPC : compter les messages non lus pour l'utilisateur connecté
CREATE OR REPLACE FUNCTION get_unread_messages_count()
RETURNS INTEGER AS $$
  SELECT COUNT(*)::INTEGER FROM admin_messages
  WHERE user_id = auth.uid()
  AND is_read = false
  AND deleted_by_user = false;
$$ LANGUAGE sql SECURITY DEFINER STABLE;

-- RPC : envoyer un message (admin uniquement)
...
-- RPC : récupérer les messages de l'utilisateur avec nom de l'expéditeur
...
RETURNS TABLE (
  ...
  SELECT m.id, m.admin_id,
    COALESCE(p.username, p.email, 'Admin') AS admin_name,
    m.subject, m.message, m.is_read, m.created_at
  FROM admin_messages m
  LEFT JOIN profiles p ON p.id = m.admin_id
