...
-- admin_update_profile (status, is_suspect, email)
CREATE OR REPLACE FUNCTION admin_update_profile(p_target_id UUID, p_status TEXT DEFAULT NULL, p_is_suspect BOOLEAN DEFAULT NULL, p_email TEXT DEFAULT NULL)
RETURNS JSONB AS $$
DECLARE
  v_admin_id UUID := auth.uid();
BEGIN
